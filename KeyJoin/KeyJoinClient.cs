...
/// <summary>
/// Provides the methods for creating <see cref="ICredentialsRequest">unified KeyJoin credential registration request messages</see>
/// and for handling the <see cref="CredentialsResponse">credential registration responses</see> received from the coordinator.
/// </summary>
public class KeyJoinClient
{
	public KeyJoinClient(
		...
		CancellationToken cancellationToken)
	{
		...
		{
			throw new KeyJoinCryptoException(KeyJoinCryptoErrorCode.CredentialToPresentDuplicated);
		}
		...
		CredentialsResponseValidation registrationValidationData)
	{
		...
		if (issuedCredentialCount != NumberOfCredentials)
		{
			throw new KeyJoinCryptoException(
				KeyJoinCryptoErrorCode.IssuedCredentialNumberMismatch,
		...
		{
			throw new KeyJoinCryptoException(KeyJoinCryptoErrorCode.ClientReceivedInvalidProofs);
		}
