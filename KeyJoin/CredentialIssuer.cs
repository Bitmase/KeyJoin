...
/// requested by a KeyJoin client. This means this component abstracts receives
...
/// proving to the KeyJoin client that the credentials were issued with the right
/// key).
///
...
public class CredentialIssuer
{
	...
	public CredentialIssuer(
		...
		BitmaseRandom randomNumberGenerator,
		long maxAmount)
	{
		MaxAmount = maxAmount;
	...
	private BitmaseRandom RandomNumberGenerator { get; }
	...
	/// This parameter is called `k` in the KeyJoin paper.
	/// </summary>
	...
	/// <exception cref="KeyJoinCryptoException">Error code: <see cref="KeyJoinCryptoErrorCode">KeyJoinErrorCode</see></exception>
	...
	public CredentialsResponse HandleRequest(ICredentialsRequest registrationRequest)
	{
		...
		{
			throw new KeyJoinCryptoException(
				KeyJoinCryptoErrorCode.InvalidNumberOfRequestedCredentials,
		...
		{
			throw new KeyJoinCryptoException(
				KeyJoinCryptoErrorCode.InvalidNumberOfPresentedCredentials,
		...
		{
			throw new KeyJoinCryptoException(KeyJoinCryptoErrorCode.InvalidBitCommitment);
		}
		...
		{
			throw new KeyJoinCryptoException(KeyJoinCryptoErrorCode.SerialNumberDuplicated);
		}
		...
		lock (SerialNumbersLock)
		{
			...
			{
				throw new KeyJoinCryptoException(KeyJoinCryptoErrorCode.SerialNumberAlreadyUsed, $"Serial number reused");
			}
		...
		try
		{
			...
			if (!areProofsValid)
			{
				throw new KeyJoinCryptoException(KeyJoinCryptoErrorCode.CoordinatorReceivedInvalidProofs);
			}
		}
		finally
		{
